...
最初に deterministic helper で active marker を要求する。agent は cwd-hash や marker path を inline shell で組み立てない:
...
helper の stdout が plan path。exit 0 の場合のみ Step 1 へ進む。

| 状態 | 動作 |
|---|---|
| helper exit 0 | stdout の plan path を読み Step 1 へ進む |
| `.active` expired | helper stderr の「.active marker expired. Run `$plan <request>` again.」を表示して終了 |
| pending only | helper stderr の「Plan exists but is not approved. Type `$impl` as a top-level prompt to approve.」を表示して終了 |
| pending expired | helper stderr の「.pending marker expired. Run `$plan <request>` again.」を表示して終了 |
| absent | helper stderr の「Run `$plan <request>` first. No active plan for this cwd.」を表示して終了 |

承認の唯一のルートは UserPromptSubmit hook (`codex-impl-approval-tracker.ts`) — ユーザーが `$impl` を top-level prompt として打鍵した時のみ helper の promote 処理で `.pending-` → `.active-` へ promote される。AI が同じ skill body 内で `$impl` 相当の何かを連想しても hook は発火しない。

## Step 1: プラン読み込み + サイドカー JSON 整合性チェック

1. Approval gate の `require-active` stdout をプラン file path として保持する (`.active-<hash>` path は agent が組み立てない)
...
Step 4 で spawn した reviewer subagent は、main session が `agent_id / role / stage / attempt / status / closed` の ledger で追跡する。各 reviewer の出力から verdict と blocker を抽出し終えたら、その reviewer は result-integrated と見なし、次 stage または retry に進む前に `close_agent` で閉じる。`close_agent` は result-integrated または terminal/known completed reviewer にだけ使い、実行中の reviewer を打ち切る用途では使わない。

`agent thread limit reached` で reviewer spawn が失敗した場合だけ、known completed / terminal reviewers を `close_agent` で閉じ、失敗した reviewer dispatch を retry exactly once する。retry 後も失敗する場合は追加 spawn を繰り返さず、その section を FAIL として扱う。
...
各 attempt の `code-reviewer` は verdict 処理後に必ず `close_agent` する。FAIL で retry する場合も、前 attempt の reviewer を閉じてから fresh `code-reviewer` を spawn する。
...
Combined Generic Review PASS 後、`REVIEW_FILES` と diff / untracked file contents をローカル検査した trigger flags から該当する specialist reviewer を **全て**選ぶ。単一選択や優先順位 cutoff はしない。ただし dispatch は bounded batch とし、max 3 concurrent reviewers を上限にする。
...
各 specialist には `REVIEW_FILES`、tracked aggregated diff、untracked file list、必要に応じて local trigger scan で得た trigger labels だけを渡す。raw untracked file contents や本文 excerpt は subagent prompt に渡さない。出力に `VERDICT: FAIL`、non-empty `MUST_FIX` section、No VERDICT、malformed output があれば Domain section は FAIL。

Domain dispatch は `AGENTS` の順序を保ったまま max 3 concurrent の batch に分割する。各 batch では最大 3 reviewer を並列 spawn し、全員の結果を処理したら次 batch に進む前に batch 内の result-integrated reviewers をすべて `close_agent` する。Domain retry では、PASS 済み reviewer は再実行せず、FAIL した specialist だけを fresh instance で再 dispatch する。この retry も max 3 concurrent batch と `close_agent` cleanup を守る。該当 reviewer なしなら `SECTION_VERDICT: PASS (no domain trigger)`。
...
`security-auditor` を spawn する前に Domain reviewer の ledger がすべて closed であることを確認する。Security verdict を処理したら、PASS / FAIL にかかわらず `security-auditor` を `close_agent` する。
...
| 状態 | 動作 |
|---|---|
| `AUDIT_VERDICT: PASS` + `REVIEW_VERDICT: PASS` | helper の `complete` でサイドカー JSON 末尾エントリ `Final Audit + Review` を completed に、`update_plan` も同じ、`deno run --allow-env=HOME --allow-read="$HOME/.codex/plans,$PWD" --allow-write="$HOME/.codex/plans" --no-prompt ~/.codex/scripts/codex-plan-marker.ts clear-active "$PWD"` で active marker を削除、最終レポート (changed files / tests added / 差分サマリ) を表示 |
...
- **fresh subagent review**: Step 4 は main session の自己レビューではなく、`~/.codex/agents/*.toml` の reviewer subagent を fresh context で起動する。Spec / Quality は combined generic review として 1 つの fresh `code-reviewer` で確認し、Domain specialists は max 3 concurrent の bounded batch で dispatch する。各 reviewer は verdict 処理後に `close_agent` で閉じ、通常時の live reviewers を bounded に保つ。
