...
The reviewer template below assumes a non-empty `{audit_verdict_input}`.
...
You have not seen any other review of this change. Judge each criterion from the code: a FAIL cites file:line, and a PASS names what you checked.
...
- `[live]` — observed by the main session on the real surface and recorded in the plan's evidence sidecar; covered by the Audit Verdict Input, not re-run here
...
- Leaving a criterion unaddressed — every criterion gets a verdict
