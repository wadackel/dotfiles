...
4. For each task:
   ...
   3. **Implement** — follow the plan's "Files to Change" and "Patterns to Mirror" exactly. Match the naming, error handling, and conventions captured by Phase 2 EXPLORE. For a task whose acceptance criteria name a failing test first (the `tdd-workflow` red step), run that red step before the green step and keep both outputs in the evidence, labelled `red (expected FAIL)` and `green (expected PASS)`; acceptance is judged on the green output, and both outputs follow step 4's redaction rule
...
5. After all implementation tasks complete, the final `Run /completion-audit and /subagent-review` task unblocks automatically. Execute in this order:
   ...
   2. On **VERIFIED PASS** (self-audit or subagent), invoke `/subagent-review` against the aggregated diff (`git diff <first-task baseline_sha>..HEAD`). `/subagent-review` runs one parallel wave of Spec & Quality, Domain specialists, and Security, then re-review waves (diagnostic for SHOULD_FIX-only failures) until every reviewer passes within its 3-round budget internally. The Security step replaces the previous standalone Security Sweep. It must return PASS (no open MUST_FIX) to close the gate
