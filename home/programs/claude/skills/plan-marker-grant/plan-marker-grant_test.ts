...
Deno.test("CLI: activate writes marker and returns JSON without cwdHash field", async () => {
  await withTempEnv(async (home) => {
    ...
    assertMatch(
      parsed.marker.path,
      /\/\.bypass-plan-gate-[0-9a-f]{32}\.json$/,
    );
    // cwdHash field must not be present in the return shape
    assertEquals(parsed.marker.cwdHash, undefined);
    assertEquals(typeof parsed.marker.sessionHash, "string");
...
Deno.test("CLI: empty subcommand defaults to activate", async () => {
  await withTempEnv(async (home) => {
    const result = await runCli(home, ["", "session-1"]);
    assertEquals(result.code, 0);
    const parsed = JSON.parse(result.stdout);
    assertEquals(parsed.status, "activated");
  });
...
Deno.test("CLI: missing session_id → exit 1", async () => {
  await withTempEnv(async (home) => {
    const result = await runCli(home, ["activate", ""]);
...
Deno.test("CLI: unknown subcommand → exit 1", async () => {
  await withTempEnv(async (home) => {
    const result = await runCli(home, ["explode", "session-1"]);
    assertEquals(result.code, 1);
    assertMatch(result.stderr, /unknown subcommand/);
  });
});

Deno.test("CLI: status after activate → valid:true (no cwdHash in return)", async () => {
  await withTempEnv(async (home) => {
    const activate = await runCli(home, ["activate", "session-1"]);
    assertEquals(activate.code, 0);
    const status = await runCli(home, ["status", "session-1"]);
...
Deno.test("CLI: clear removes marker", async () => {
  await withTempEnv(async (home) => {
    ...
    const clear = await runCli(home, ["clear", "session-1"]);
...
Deno.test("CLI: dangerous session_id does not leak into path", async () => {
  await withTempEnv(async (home) => {
    ...
    const result = await runCli(home, ["activate", danger]);
    assertEquals(result.code, 0);
    const parsed = JSON.parse(result.stdout);
