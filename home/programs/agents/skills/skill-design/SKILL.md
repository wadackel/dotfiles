...
2. **User-facing print strings inside fenced code blocks or quoted message templates**: Strings that the agent prints verbatim to the user (confirmation prompts, option labels, sample template headers) may stay in the user's language, typically Japanese for jp users. Keep them inside fenced code blocks or backticks so they are visually distinguishable from instruction prose.
