...
metadata:
  author: GoogleChrome
  version: "0.0.169"
  upstream: https://github.com/GoogleChrome/modern-web-guidance
---
...
Search with an action-oriented query summarizing what you want to achieve using the `search` command. Run the `modern-web-guidance` binary directly (installed locally via mise).

```sh
modern-web-guidance search "<query>"
```
...
- The `modern-web-guidance` binary is installed and managed by mise (see `home/programs/mise/default.nix`). It runs locally without network access at command time.
- If `modern-web-guidance` is not found on PATH, run `mise install` to materialize it. The mise activation hook in this dotfiles environment runs this automatically on `darwin-rebuild`.
- The version is pinned in `home/programs/mise/default.nix` (matching `metadata.version` above). To upgrade, bump that pin together with the `metadata.version` field, then run `mise install`.
