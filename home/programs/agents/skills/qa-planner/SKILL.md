...
- **Mode C (Agent Team QA)**: Large-scale verification where parallel execution is beneficial.
  ...
  **Mode C structure (Lead + QA Tester as base):**
  1. Spawn a named QA Tester (Agent tool, `general-purpose`, with `name`) for agent-browser testing
  ...
  6. When verification is done, stop every named agent with `TaskStop`
...
- **State-import + headless + per-Claude-session daemon**: every `agent-browser` invocation must carry two flags — `--session "claude-$PPID"` (isolates the daemon to this Claude session, so parallel Claude instances cannot kill each other's daemon) and `--state "$HOME/.agent-browser-state/main.json"` on the first call (loads the plaintext state file written by `abr`). Subsequent calls within the same Claude session can omit `--state`. Lead and QA Tester subagents share the same daemon (same `$PPID`) — neither side should `close` until the workflow is complete. The user's live Chrome window is not touched, eliminating window-collision risk during QA execution.

If a browser command fails with `No such file or directory: .../main.json`, the state file has not been imported yet. Stop and ask the user to run `abr` against a logged-in Chrome (see agent-browser `references/authentication.md`). Do not fall back to attaching the user's live Chrome — that re-introduces the collision risk this default eliminates.
...
Run the setup block below as the first Bash call of Step 4, before any `agent-browser`, `curl`, `screenshot`, or other test command: screenshots and `report.md` are written under `$RUN_DIR`, so a test that runs before `mkdir -p "$RUN_DIR/screenshots"` succeeds leaves no evidence on disk.

If `mkdir -p` fails (permission denied, disk full, etc.), stop Step 4 and report the failure to the user instead of running tests whose evidence could not be saved.
...
**Video recording is not used.** `agent-browser record start` clears the current tab's localStorage (observed on agent-browser 0.31), which logs out localStorage-backed auth SPAs and redirects them to their login page. Screenshots and `report.md` are the evidence; do not add `record start` / `record stop` to this workflow.
...
2. Stop when the charter is answered or new interactions stop surfacing new behavior
...
Mode B output needs the `$RUN_DIR` created by Step 4 setup. If its literal value is not known (setup was skipped, or the value was lost between Bash calls), run Step 4 "Run directory setup" first; without it the report cannot be written to disk and only a chat copy would remain.

Produce Mode B output in this order, so the saved report exists before the chat copy does:

1. **Persist first (Bash)** — write the Mode B body to `$RUN_DIR/report.md` via the heredoc below, before showing it in chat.
2. **Verify on disk (Bash)** — run `ls -la "$RUN_DIR/report.md" && wc -l "$RUN_DIR/report.md"` so the file's existence and size appear in chat. If this command fails or shows an empty file, redo substep 1 before continuing.
