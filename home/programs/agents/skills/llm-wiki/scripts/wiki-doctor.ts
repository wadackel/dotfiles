...
const VAULT = argOf("--vault", Deno.env.get("LLM_WIKI_VAULT_ROOT") ?? "").replace(/\/+$/, "");
...
if (!VAULT) {
  console.error("LLM_WIKI_VAULT_ROOT が未設定。--vault で指定するか環境変数を設定してください。");
  Deno.exit(2);
}
...
const add = (name: string, bad: string[], okMsg: string) =>
  checks.push({ name, ok: bad.length === 0, detail: bad.length ? bad : [okMsg] });
...
{
  const isDate = (t: string) => /^\d{4}-\d{2}-\d{2}$/.test(t) || /^\d{4}-W\d{1,2}$/.test(t);
...
{
  const notes = new Set(mdFiles.filter((p) => rel(p).startsWith("02_Notes/")).map((p) => p.split("/").pop()!));
  const bad = mdFiles
    .filter((p) => rel(p).startsWith("98_Maintenance/"))
    .filter((p) => notes.has(p.split("/").pop()!))
    .map((p) => `${rel(p)} は 02_Notes/ の同名ノートと衝突（wikilink が自己参照になる）`);
  add("98_Maintenance と 02_Notes のファイル名が衝突していない", bad, "衝突なし");
}
...
// ---- 3. 知識マップがコードフェンスで囲まれていない ----
{
  ...
  for (const [p, body] of bodies) {
    ...
    if (!map) continue;
    if (/^```/m.test(map)) bad.push(`${rel(p)} の知識マップがフェンス内（wikilink が不活性）`);
  }
...
const written = new Set<string>();
{
  for (const p of mdFiles) {
    const r = rel(p);
    if (r.startsWith("98_Maintenance/lint-report-") || r.startsWith("98_Maintenance/proposals/") ||
        r.startsWith("98_Maintenance/logs/")) written.add(p);
  }
  if (BASELINE_DIR) {
    for (const p of mdFiles) {
      const r = rel(p);
      if (!r.startsWith("02_Notes/") && !r.startsWith("04_Literature/")) continue;
      try {
...
// ---- 4. lint の成果物が生の wikilink で監査対象を引用していない ----
{
  ...
  for (const [p, body] of bodies) {
    ...
    if (!r.startsWith("98_Maintenance/lint-report-") && !r.startsWith("98_Maintenance/proposals/")) continue;
    ...
    if (hits.length) bad.push(`${r} が生の wikilink を ${hits.length} 件含む: ${hits.slice(0, 3).join(", ")}`);
  }
  add("lint レポートと proposals が生の wikilink を含まない", bad, "全てコード表記");
}
...
{
  const privateNames = allFiles.filter(isPrivate).map((p) => p.split("/").pop()!.replace(/\.md$/, ""));
  ...
  for (const p of await walk(SKILL)) {
    ...
    try { targets.set(`(skill) ${p.slice(SKILL.length)}`, await Deno.readTextFile(p)); } catch { /* 読めなければ飛ばす */ }
  }
  try { targets.set("CLAUDE.md", await Deno.readTextFile(`${VAULT}/CLAUDE.md`)); } catch { /* 無ければ飛ばす */ }
  ...
  const preexisting = async (label: string) => {
    ...
    try { return await Deno.readTextFile(`${BASELINE_DIR}/${label}`); } catch { return ""; }
  };
  ...
  for (const [label, body] of targets) {
    ...
    if (names) bad.push(`${label}: 05_Private のファイル名を ${names} 件含む（名前は伏せる）`);
    ...
    if (paths) bad.push(`${label}: 05_Private 配下の具体パスを ${paths} 件含む`);
  }
  add("05_Private の名前と具体パスが成果物・仕様のどちらにも漏れていない", bad,
...
// ---- 6. sources / related / generated_pages が JSON として妥当 ----
{
  ...
  for (const [p, body] of bodies) {
    ...
    for (const key of ["sources", "related", "generated_pages"]) {
      ...
      if (!seq) continue;
      try { JSON.parse(seq); } catch { bad.push(`${rel(p)} の ${key} が不正な YAML flow sequence`); }
    }
  }
...
{
  const logText = mdFiles.filter((p) => rel(p).startsWith("98_Maintenance/logs/"))
  ...
  for (const [p, body] of bodies) {
    ...
    if (!gp.length) { bad.push(`${rel(p)}: type: source だが generated_pages が空`); continue; }
    if (!gp.some((g) => logText.includes(`[[${g}]]`))) bad.push(`${rel(p)}: generated_pages の宛先がログに無い`);
  }
...
// ---- 8. 双方向リンク（generated_pages ↔ sources） ----
{
  ...
  add("generated_pages と sources が双方向で一致",
    [...onlyFwd.map((k) => `generated_pages のみ: ${k}`), ...onlyRev.map((k) => `sources のみ: ${k}`)].slice(0, 20),
    `${fwd.size} 対が両方向で一致`);
}
...
// ---- 9. 本文の 関連ページ / ソース が frontmatter の部分集合 ----
{
  ...
  for (const [p, body] of bodies) {
    ...
    const sec = (h: string) => prose.split(new RegExp(`^## ${h}$`, "m"))[1]?.split(/^## /m)[0] ?? "";
    for (const m of sec("関連ページ").matchAll(/\[\[([^\]|]+)/g)) {
      if (related.size && !related.has(m[1].trim())) bad.push(`${rel(p)}: 関連ページの ${m[1].trim()} が related に無い`);
    }
    for (const m of sec("ソース").matchAll(/\[\[([^\]|]+)/g)) {
      if (sources.size && !sources.has(m[1].trim())) bad.push(`${rel(p)}: ソースの ${m[1].trim()} が sources に無い`);
    }
  }
...
// 同じ不変条件が複数ファイルに書かれている以上、片方だけ直す事故が起きる。
{
  ...
  for (const p of specFiles) {
    let t: string;
    try { t = await Deno.readTextFile(p); } catch { continue; }
    // ログのパスは ` 操作ログ` サフィックス付きでなければならない
    for (const m of t.matchAll(/logs\/(<MOC>|<genre>)([^\s`)]*)\.md/g)) {
      if (!m[2].includes("操作ログ")) bad.push(`${p.replace(Deno.env.get("HOME") ?? "", "~")}: ログパスが旧形式 (${m[0]})`);
    }
  }
  add("ログパスの表記が全仕様ファイルで一致", bad, "全ファイルで ` 操作ログ` 形式");
}
...
{
  const noteName = (p: string) => p.slice(p.lastIndexOf("/") + 1).replace(/\.md$/, "");
  const notes = new Map<string, string>();
  for (const p of mdFiles) if (rel(p).startsWith("02_Notes/")) notes.set(noteName(p), bodies.get(p)!);
  ...
  const queue = [...stripCode(home).matchAll(LINK)].map((m) => m[1].trim()).filter((n) => notes.has(n));
  const reached = new Set(queue);
  while (queue.length) {
    for (const l of bodyLinks(queue.shift()!)) {
      if (notes.has(l) && !reached.has(l)) { reached.add(l); queue.push(l); }
    }
  }
  ...
  for (const [name, text] of notes) {
    ...
    if (fm.split("\n").some((l) => l === "type: concept")) {
      bad.push(`02_Notes/${name}.md: type: concept だが Home から本文リンクで辿れない`);
    } else legacy++;
  }
  add(
...
// 失敗にはしない — 未コンパイルの記事が残っていること自体は正常な状態。
{
  ...
  const note = nested.size ? `（うちサブディレクトリ: ${[...nested].join(", ")}）` : "";
  add("未コンパイル記事の本数を再帰的に数えている", [], `未コンパイル ${uncompiled} 本${note}`);
}
...
for (const c of checks) {
  ...
  if (c.detail.length > 15) console.log(`       … 他 ${c.detail.length - 15} 件`);
}
