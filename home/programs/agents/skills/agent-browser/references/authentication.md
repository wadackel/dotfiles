...
- No application-layer encryption: env-var-derived keys give no protection against same-UID readers, who can read the env directly, and the machinery it needs (secret-manager lookups, encrypted-file suffixes, biometric prompts on shell startup) adds nothing FileVault does not already cover.
