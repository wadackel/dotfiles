...
- **`permissions.allow`**: `Edit(~/.claude/**)` allows skills to edit existing files under `~/.claude/` without confirmation dialogs. New-file creation is deliberately narrower: only `Write(~/.claude/plans/**)` is allowed, so plan files never stall on a permission prompt while `settings.json` / `bash-policy.yaml` overwrites still require confirmation
